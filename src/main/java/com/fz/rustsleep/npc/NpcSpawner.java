...
public class NpcSpawner {
    ...
    public void despawnForPlayer(SleepingBody body, Player viewer) {
        ...
        send(viewer,
            new WrapperPlayServerDestroyEntities(body.getNpcEntityId())
        );
    ...
    public void updateHealthForAll(SleepingBody body) {
        ...
        double dist =
            plugin.getConfig().getDouble(
                "settings.name-visible-distance",
                8.0
            );
        ...
        for (Player p : Bukkit.getOnlinePlayers()) {
            ...
            boolean nearby =
                p.getWorld().equals(body.getLocation().getWorld())
                    && p.getLocation()
                        .distanceSquared(body.getLocation()) <= distSq;
    ...
    private void sendTabAdd(SleepingBody body, Player viewer) {
        ...
        if (body.getSkinTexture() != null
            && body.getSkinSignature() != null) {
    ...
    private void sendEquipment(SleepingBody body, Player viewer) {
        ...
        addEquip(list, EquipmentSlot.CHEST_PLATE, body.getChestplate());
